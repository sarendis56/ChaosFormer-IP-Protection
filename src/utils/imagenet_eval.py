...
from typing import Tuple, Dict, List, Optional, Callable, Any, cast
import numpy as np
...
class ImageNetValidationDataset(Dataset):
    def __init__(self, image_dir, validation_synsets, processor):
        ...
        if not os.path.exists(image_dir):
            raise FileNotFoundError(f"ImageNet validation directory not found at {image_dir}")
        ...
        if not self.image_files:
            raise RuntimeError(f"No ImageNet validation images found in {image_dir}. Please ensure dataset is setup correctly.")
...
def evaluate_topk_accuracy(
    ...
    topk: Tuple[int, ...] = (1, 5),
    logits_getter: Optional[Callable[[Any], torch.Tensor]] = None,
) -> Dict[int, float]:
    """Evaluate top-k accuracy for generic classifiers.

    Supports models that return either:
    - raw logits tensor, or
    - an object with a `.logits` attribute (e.g., HF models).
    """
    model.eval()
    device_obj = torch.device(device)
    requested_topk = tuple(sorted(set(int(k) for k in topk if int(k) > 0)))
    if not requested_topk:
        raise ValueError("topk must contain at least one positive integer")

    correct_by_k = {k: 0.0 for k in requested_topk}
    total = 0
    ...
    for images, labels in dataloader:
        ...
        if logits_getter is not None:
            logits = logits_getter(outputs)
        elif isinstance(outputs, torch.Tensor):
            logits = outputs
        elif hasattr(outputs, "logits"):
            logits = outputs.logits  # type: ignore[assignment]
        else:
            raise TypeError("Model output must be a Tensor, expose `.logits`, or use logits_getter")

        max_supported_k = min(max(requested_topk), logits.shape[1])
        _, pred = torch.topk(logits, k=max_supported_k, dim=1)
        pred = pred.transpose(0, 1)
        correct = pred.eq(labels.view(1, -1).expand_as(pred))

        for k in requested_topk:
            k_eff = min(k, max_supported_k)
            correct_by_k[k] += correct[:k_eff].any(dim=0).float().sum().item()
        total += labels.size(0)

    return {k: 100.0 * correct_by_k[k] / float(max(1, total)) for k in requested_topk}
...
if __name__ == "__main__":
    ...
    model = cast(torch.nn.Module, ViTForImageClassification.from_pretrained(model_name))
    ...
    # Run validation
    top1_acc, top5_acc, class_accuracies = validate_model(
        model,
        dataset,
        device,
        synset_to_name,
