...
def get_optimal_arnold_functions(device: torch.device, use_triton: bool = False):
    """Return the optimized Arnold Cat Map functions for the given device."""
    if device.type == 'cuda':
        if use_triton:
            return arnold_triton, iarnold_triton
...
class InferenceOverheadAnalyzer:
    ...
    def __init__(self, model_name: str = "google/vit-base-patch16-224", device: str = "auto", batch_size: int = 1, skip_imagenet: bool = False, use_triton: bool = False, master_secret: Optional[str] = None):
        ...
        if master_secret:
            ...
            print(f"✓ Derived ACM and Permutation keys from master secret: {master_secret[:4]}...{master_secret[-4:]}")
        else:
        ...
        self.arnold_encrypt, self.arnold_decrypt = get_optimal_arnold_functions(self.device, use_triton=self.use_triton)
    ...
    def _time_realistic_encrypted_inference(self, encrypted_layers: List[int], acm_key: Optional[List[int]] = None, 
        ...
        # Use derived ACM key if none provided
        if acm_key is None:
            acm_key = self.derived_acm_key
    ...
    def analyze_acm_iterations_overhead(self, max_iterations: int = 20) -> List[TimingBreakdown]:
        ...
        encrypted_layers = list(range(6))  # Fixed number of layers
        
        results = []
        for n_iter in tqdm(acm_iterations):
            # Keep the derived a, b, c, d parameters but vary the iterations N
            key = [n_iter] + self.derived_acm_key[1:]
    ...
    def analyze_layers_overhead(self, max_layers: int = 12) -> List[TimingBreakdown]:
        ...
        for num_layers in tqdm(layer_counts):
            ...
            timing_result = self._time_realistic_encrypted_inference(encrypted_layers, self.derived_acm_key, num_runs=None)
            results.append(self._timing_result_to_breakdown(timing_result))
    ...
    def _create_stacked_bar_chart(self, ax, results: List[TimingBreakdown], x_labels: List[int], 
        ...
        ax.bar(x_positions, acm_enc_times, width=bar_width, bottom=forward_times, 
        ...
        total_times = [f + ae + ad + fe + fd for f, ae, ad, fe, fd in 
                      zip(forward_times, acm_enc_times, acm_dec_times, ffn_enc_times, ffn_dec_times)]
    ...
    def create_overhead_breakdown_plot(self,
        ...
        n_iterations = [3, 5, 8, 12, 16, 32]
        n_layers = [2, 4, 6, 8, 10, 12]

        self._create_stacked_bar_chart(axes[0, 0], acm_results, n_iterations, 
                                       'Number of ACM Iterations', show_legend=True)
        self._create_stacked_bar_chart(axes[0, 1], layers_results, n_layers, 
                                       'Number of Encrypted Layers', show_legend=False)
        self._create_stacked_bar_chart(axes[1, 0], acm_results_batch64, n_iterations, 
                                       'Number of ACM Iterations', show_legend=True)
        self._create_stacked_bar_chart(axes[1, 1], layers_results_batch64, n_layers, 
                                       'Number of Encrypted Layers', show_legend=False)
    ...
    def save_overhead_data(self, acm_results: List[TimingBreakdown],
        ...
        acm_data = [self._timing_to_dict(r, n_iterations=n) 
                    for r, n in zip(acm_results, acm_iterations)]
    ...
    def create_batch64_only_plot(self,
                                ...
                                device_name: str = "GPU",
                                output_dir: str = "results/analysis"):
        """Create a separate figure with only batch size 64 plots."""
        ...
        self._create_stacked_bar_chart(axes[0], acm_results_batch64, n_iterations, 
                                       'Number of ACM Iterations', show_legend=True)
    ...
    def regenerate_plots_from_csv(self, device_name: str = "gpu", output_dir: str = "results/analysis", include_batch64: bool = True):
        ...
        acm_csv = base_path / f"acm_iterations_overhead_{device_name.lower()}.csv"
        layers_csv = base_path / f"layers_overhead_{device_name.lower()}.csv"

        if not acm_csv.exists() or not layers_csv.exists():
            print(f"Error: CSV files not found in {output_dir}")
            print(f"Expected files: {acm_csv.name} and {layers_csv.name}")
            return

        acm_results = self._load_results_from_csv(acm_csv)
        ...
        if include_batch64:
            acm_csv_b64 = base_path / f"acm_iterations_overhead_{device_name.lower()}_batch64.csv"
            layers_csv_b64 = base_path / f"layers_overhead_{device_name.lower()}_batch64.csv"
            
            if acm_csv_b64.exists() and layers_csv_b64.exists():
                acm_results_batch64 = self._load_results_from_csv(acm_csv_b64)
