...
class InferenceOverheadAnalyzer:
    ...
    FONT_SIZES = {
        ...
        'axis_labels': 36,     # X and Y axis labels
        'subtitles': 28,       # Subplot titles (a, b, c, d)
    ...
    def _time_realistic_encrypted_inference(self, encrypted_layers: List[int], acm_key: List[int], 
        ...
        if self.device.type == 'cuda':
            ...
            sample_xor_weight = torch.randn(768, 768, device=self.device)
            
            for _ in range(3):
                # Use optimized version for warmup
                xor_encrypt_decrypt(sample_xor_weight, layer_idx=0, weight_name='test',
                                   seed_base=42, in_place=True)
                xor_encrypt_decrypt(sample_xor_weight, layer_idx=0, weight_name='test',
                                   seed_base=42, in_place=True)  # Second call to decrypt back
                self._synchronize()
    ...
    def create_overhead_breakdown_plot(self,
                                     ...
                                     layers_results_batch64: Optional[List[TimingBreakdown]] = None,
                                     device_name: str = "GPU",
        ...
        if layers_results_batch64 is not None:
            ...
            # Single subplot: batch size 1 vs encrypted layers
            fig, ax = plt.subplots(1, 1, figsize=(18, 8))
            plt.subplots_adjust(top=1, hspace=0.3, wspace=0.2)

            self._create_stacked_bar_chart(ax, layers_results, n_layers,
                                           'Number of Encrypted Layers', show_legend=True)
